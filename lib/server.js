...
exports.run = function (config) {
  ...
  .on('connection', function (socket) {
    ...
    socket.on('getUsers', function (data) {
      var userId = parseInt(data.id, 10);
        var listUsers = [];
        if(rooms[currentRoom][userId]){
          for (var key in rooms[currentRoom]){
            var user = rooms[currentRoom][key].username;
            if(user != null)
            listUsers.push(user);
          }
          rooms[currentRoom][userId].emit('getUsers', listUsers);
        }else{
    ...
    socket.on('joinRoom', function (data) {
      ...
      var roomsRouting = [];
        for(var key in rooms){
            var addedRoom = [];
            if(rooms[key].name == data.addRoom){
              var roomObject = {
                token: key,
                name: data.addRoom
              };
        ...
        var currentRoomObject = {
          token: currentRoom,
          name: rooms[currentRoom].name
        };
        rooms[currentRoom][userId].emit('joinRoom', roomsRouting);
    });
    ...
    //           };
    //           roomsRouting.push(roomObject);
    //           rooms[currentRoom][userId].emit('leaveRoom', roomsRouting);
    //         }
    //     }
    //     delete[rooms][currentRoom][userId].indexOf(socket);
    // });
